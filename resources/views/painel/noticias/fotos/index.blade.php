...
@section('content')
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-12">
                <a data-href="{{url('painel')}}/noticias/{{Route::input('id_not')}}/fotos/multiploupload"
                                    ...
                                    @foreach($itens as $item)
                                        <tr>
                                            <td class="text-center" style="vertical-align: middle; position: relative;">
                                                <img src="{{asset("/upload/noticias/p_".$item->FotCodigo.".jpg?cache=".date("Ymdhis", strtotime($item->updated_at)))}}"
                                                ...
                                                <div style="text-align: left !important;position: absolute; bottom: 0px; left:0px; padding: 3px;background-color: rgba(255,255,255, 0.9); width: 100%;">
                                                    <a href="{{getenv("PAINEL")}}/noticias/{{Route::input('id_not')}}/fotos/{{$item->FotCodigo}}/destaque"
                                                    ...
                                                    <a data-href="{{getenv("PAINEL")}}/noticias/{{Route::input('id_not')}}/fotos/{{$item->FotCodigo}}/upload" class="btn btn-sm btn-primary  btn-upload-multiplo-modal" data-toggle="tooltip" data-placement="right" title="" data-original-title="Alterar foto"><i
                                            ...
                                            <td style="vertical-align: middle">
                                                <div class="form-group form{{$item->FotCodigo}}">
                                                    <input type="text" class="form-control legenda" maxlength="300" value="{{$item->FotLegenda}}" name="legenda" id="legenda"
                                                           data-id="{{$item->FotCodigo}}" data-href="{{getenv("PAINEL")}}/noticias/{{Route::input('id_not')}}/fotos/{{$item->FotCodigo}}/legenda">
                                                    <span class="help-block span{{$item->FotCodigo}} hidden"></span>
                                                </div>
                                            </td>
                                            <td class="text-center" style="vertical-align: middle">
                                                <a href="{{getenv("PAINEL")}}/noticias/{{Route::input('id_not')}}/fotos/{{$item->FotCodigo}}/destroy?{{http_build_query(Input::all())}}"
